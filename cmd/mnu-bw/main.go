...
func clearClipboardSubcommand() {
    ...
    for i := range contentBytes { contentBytes[i] = 0 }

    timeoutSeconds, err := strconv.Atoi(os.Args[1])
    if err != nil { fmt.Printf("Invalid timeout: %v\n", err); os.Exit(1) }
    uniqueID := os.Args[2]

    configDir, err := util.GetConfigDir()
    if err != nil { fmt.Printf("Failed to get config directory: %v\n", err); os.Exit(1) }
    ...
    go func() {
        ...
        if err != nil { fmt.Printf("Failed to open FIFO for reading: %v\n", err); return }
        defer fifo.Close()
        buf := make([]byte, 1)
        _, err = fifo.Read(buf)
        if err != nil && err != io.EOF { fmt.Printf("Error reading from FIFO: %v\n", err); return }
        close(cancelChan)
    }()
    ...
    case <-time.After(time.Duration(timeoutSeconds) * time.Second):
        ...
        if err != nil { fmt.Printf("Failed to read clipboard for sanity check: %v\n", err); os.Exit(1) }
        curHash := sha256.Sum256([]byte(current))
        if curHash != origHash { fmt.Println("Clipboard content changed; skipping clear."); return }
        if err := clipboard.WriteAll(""); err != nil { fmt.Printf("Failed to clear clipboard: %v\n", err); os.Exit(1) }
    case <-cancelChan:
...
func bitwardenMain() {
    ...
    if err != nil { fmt.Println("mnu-bw is already running; exiting."); os.Exit(0) }
    ...
    if err != nil { fmt.Printf("Alas, there's been an error: %v\n", err); os.Exit(1) }

    sessionKey, err := keychain.GetSessionKey()
    if err == nil && sessionKey != "" { os.Setenv("BW_SESSION", sessionKey) }
    ...
    go func() {
        <-c
        if bwServeCmd != nil { bwServeCmd.Process.Kill() }
        os.Exit(0)
    }()

    p := tea.NewProgram(uipkg.InitialModel(bwManager, config))
    if _, err := p.Run(); err != nil { fmt.Printf("Alas, there's been an error: %v\n", err); os.Exit(1) }
    if bwServeCmd != nil { bwServeCmd.Process.Kill() }
}
